...
class RunMcM():
    ...
    def remove_request_checker(self):
      print "--remove request checker--"
      f=open('test.sh','r')
      fnew=open('test.sh_new','w')
      
      lines=f.readlines()
      for line in lines:
        if 'python request_fragment_check.py' in line:
          line='#'+line
        fnew.write(line)
      f.close()
      fnew.close()
      os.system('mv test.sh_new test.sh')
    def get_test_command(self):
        PREPID=self.PREPID
        #https://cms-pdmv.cern.ch/mcm/public/restapi/requests/get_test/
        url = 'https://cms-pdmv.cern.ch/mcm/public/restapi/requests/get_test/'+PREPID
        ...
        os.system('mv '+PREPID+' test.sh')
	#convert_arch('setup.sh')
        os.system('chmod u+x test.sh')
        #subprocess.call(os.getcwd()+'/test.sh',shell=True)
        #os.system('source '+os.getcwd()+'/test.sh')
...
queue
	...
	ncpu='8'
        ...
        ## exe.sh which runs test.sh addtime source myrun
        f=open(shname,'w')
        f.write('#! /bin/bash\n')
        f.write('source test.sh\n')
        ...
        f.write('log='+logname+'\n')
        f.write('transfer_input_files=test.sh\n')
    ...
    ##cmsRun -e -j HIG-RunIISummer15wmLHEGS-03367_rt.xml HIG-RunIISummer15wmLHEGS-03367_1_cfg.py || exit $? ;
    def modify_cmsRun_python(self):
      f=open('test.sh','r')
      fnew=open('test.sh_new','w')
      lines=f.readlines()
      for line in lines:
        if 'cmsRun -e -j' in line:
          #print "Find cmsRun -e -j"
          #print self.PREPID+'_1_cfg.py', 'to','my_cfg.py' 
          line=line.replace(self.PREPID+'_1_cfg.py','my_cfg.py')
        fnew.write(line)
